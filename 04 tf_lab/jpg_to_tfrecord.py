...
flags = tf.app.flags
flags.DEFINE_string("image_dir", "Cropped", "The directory of cropped and resized dog images [Cropped]")
...
def persistence_image_data_to_tfrecords(
    ...
    for images_filename, y_label in zip(x_data, y_data):
        ...
        try:
            ...
            decoded_img = tf.image.decode_image(image_file, channels=3)   # 'jpg' file decoding
            rgb_img_float = tf.image.convert_image_dtype(decoded_img, tf.float32)   # convert to float
            gray_img = tf.image.rgb_to_grayscale(rgb_img_float) # rgb to grayscale
            ...
            # Crop image using bounding_box
            crop_img = tf.image.crop_to_bounding_box(
                gray_img,    # decoded img (tensor type)
                bounding_box[0][1], # offset_height; vertical coordinate of top-left
                bounding_box[0][0], # offset_width; horizontal coordinate of top-left
                bounding_box[0][3], # target_height; height of the result
                bounding_box[0][2], # target_width; width of the result
            )

            # Resize image (32 by 32)
            image = tf.image.resize_images(crop_img, size=[32,32])
            """
